...
```
sudo cp /vagrant/kube-controller-manager.kubeconfig /var/lib/kubernetes/
```
...
```
sudo cp /vagrant/kube-scheduler.kubeconfig /var/lib/kubernetes/
```
...
The commands in this section will effect the entire cluster and only need to be run once from the client/host where vagrant is running.
